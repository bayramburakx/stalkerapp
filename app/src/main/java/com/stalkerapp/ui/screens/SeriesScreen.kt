...
/**
 * Portio Diziler Ekranı (SeriesScreen) - Özel Dizi Kataloğu, Sezon Bilgisi ve Filtreleme
 */
@Composable
fun SeriesScreen(
    profile: Profile?,
    ...
    val filtered by androidx.compose.runtime.produceState(
        ...
        value = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Default) {
            val q = query.trim()
            var list = if (catalog.series.isNotEmpty()) catalog.series else catalog.allItems.filter { catalog.isSeriesItem(it) }
    ...
    val shownCats = remember(catList, seriesCatIds, blockedCategoryIds, catalog.series.size) {
        ...
        if (raw.isNotEmpty()) raw
            ...
            val items = if (catalog.series.isNotEmpty()) catalog.series else catalog.allItems.filter { catalog.isSeriesItem(it) }
    ...
    val sectionTitle = remember(selectedCategory, shownCats, query) {
        when {
            query.isNotBlank() -> L10n.t(lang, "Dizi ara…")
            selectedCategory == 0L -> L10n.t(lang, "Tüm Diziler")
    ...
    Column(modifier = modifier.fillMaxSize().background(PortioColors.Background)) {
        ...
        PortioSearchBar(
            ...
            placeholder = L10n.t(lang, "Dizi ara…"),
        ...
        when {
            ...
            catalog.allItems.isEmpty() && catalog.status != VodCatalogStatus.Syncing ->
                EmptyState(L10n.t(lang, "Dizi içeriği bulunamadı"))
    ...
    if (quickActionItem != null) {
        ...
        VodQuickActionsSheet(
            ...
            isSeries = true,
            vm = vm,
            onOpenDetail = { onOpenVod(qi.id, true) },
